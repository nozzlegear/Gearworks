...
export default class HomePage extends Router<IProps, IState> {
    ...
    private async toggleStatus(id: number, setStatusTo: "open" | "closed") {
        ...
        const api = new ShopifyApi(Stores.AuthStore.token);
        ...
        this.setState({ loading: false, error: undefined }, () => {
            Stores.DashboardStore.updateOrder(id, order);
        })
    }
    ...
    private async deleteOrder(id: number) {
        ...
        this.setState({ loading: false, error }, () => {
            if (!error) {
                Stores.DashboardStore.removeOrder(id);
            }
        });
    }
    ...
    public async componentDidMount() {
        ...
        try {
            orders = await api.listOrders({ limit: 100, page: 1, status: "any" });
        } catch (e) {
            const err: ApiError = e;
        ...
        this.setState({ error }, () => {
            if (!error) {
                Stores.DashboardStore.loadOrders(orders);
            }
        });
    }
    ...
    public render() {
        ...
        if (!Stores.DashboardStore.loaded) {
            body = (
            ...
            body = (
                <Table onRowSelection={rows => this.setState({ selectedRows: rows })} >
                    ...
                    <TableBody deselectOnClickaway={false}>
                        {Stores.DashboardStore.orders.map((o, i) => (
        ...
        if (this.state.selectedRows && this.state.selectedRows.length > 0) {
            const order: Models.Order = Stores.DashboardStore.orders[this.state.selectedRows[0]];
        ...
        return (
            <div>
                <section id="home" className="content">
                    <h2 className="content-title">{`Latest Orders for ${Stores.AuthStore.session.shopify_shop_name}`}</h2>
                    {body}
                </section>
                ...
                <NewOrderDialog apiToken={Stores.AuthStore.token} open={this.state.dialogOpen} onRequestClose={() => this.setState({ dialogOpen: false })} />
            </div>
        );
    }
}